...
def solve(filename: str, debug: bool = False) -> int:
    ...
    for i in range(grid.shape[1]):
        ...
        follow_beam(grid, start_pos, D.DOWN, beams)
        best = max(best, len(np.where(beams != 0)[0]))
    for i in range(grid.shape[1]):
        start_pos = (grid.shape[0] - 1, i)
        beams = np.array([[0 for _ in range(grid.shape[1])] for _ in range(grid.shape[0])])
        follow_beam(grid, start_pos, D.UP, beams)
        best = max(best, len(np.where(beams != 0)[0]))
    for i in range(grid.shape[0]):
        start_pos = (i, 0)
        beams = np.array([[0 for _ in range(grid.shape[1])] for _ in range(grid.shape[0])])
        follow_beam(grid, start_pos, D.RIGHT, beams)
        best = max(best, len(np.where(beams != 0)[0]))
    for i in range(grid.shape[0]):
        start_pos = (i, grid.shape[1] - 1)
        beams = np.array([[0 for _ in range(grid.shape[1])] for _ in range(grid.shape[0])])
        follow_beam(grid, start_pos, D.LEFT, beams)
        best = max(best, len(np.where(beams != 0)[0]))
